...
/**
 * @brief Function returns node structure by its index in graph (from 1)
...
node_t *graph_get_node_by_index(unsigned int nodeIndex) {
    if (nodeIndex - 1 >= graph->node_count) {
        error_exit(graphNodeNotFoundError, "Node index out of range\n");
    }

    return graph->nodes[nodeIndex - 1];
}

/**
 * @brief Function returns node structure of the edge connected node by index (from 0)
